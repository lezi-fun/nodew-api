...
支付相关的返回地址、provider 流程和排障说明见 [钱包充值](./wallet-topup.md)。
...
Creem 钱包充值使用在 Creem 侧创建好的固定金额产品。当前已支持向已登录用户返回安全的产品目录，为已配置产品创建 Checkout Session，并在签名 webhook 确认付款后入账。

| 变量 | 必填 | 说明 |
| --- | --- | --- |
| `APP_BASE_URL` | 开启 Creem 充值时必填 | Creem Checkout 成功跳转使用的公开控制台地址。 |
| `CREEM_TOPUP_ENABLED` | 否 | 设为 `true` 后在钱包页展示 Creem 配置可用状态，默认 `false`。 |
| `CREEM_API_KEY` | 开启时必填 | 创建 Creem Checkout 使用的 API Key，不会返回给客户端。 |
| `CREEM_WEBHOOK_SECRET` | 入账必填 | Creem webhook 签名密钥，不会返回给客户端。 |
| `CREEM_TEST_MODE` | 否 | Creem API 调用使用测试 endpoint，默认 `false`。 |
...
- `POST /api/user/topup/creem/checkout` 接收 `{ "productId": string }`，为已配置产品创建待支付充值订单，并返回 Creem Checkout URL。
- Creem webhook endpoint 配置为 `/api/user/topup/creem/webhook`。
- 已支付 Creem webhook 只会入账一次，即使 Creem 重试投递也不会重复加额度。
...
Waffo 钱包充值使用同样的固定产品配置格式。已登录用户可以读取安全产品目录，并创建托管 checkout 订单；签名 webhook 支付事件会自动入账。

| 变量 | 必填 | 说明 |
| --- | --- | --- |
| `APP_BASE_URL` | 开启 Waffo 充值时必填 | 预留用于构造 Waffo checkout 返回地址的公网控制台 URL。 |
| `WAFFO_TOPUP_ENABLED` | 否 | 设为 `true` 后在钱包页展示 Waffo 配置可用状态，默认 `false`。 |
| `WAFFO_API_KEY` | 开启时必填 | 创建 Waffo Checkout 使用的 API Key，不会返回给客户端。 |
| `WAFFO_PRIVATE_KEY` | 开启时必填 | 用于签名 checkout 创建请求的 Base64 PKCS#8 DER RSA 私钥。 |
| `WAFFO_PUBLIC_KEY` | 入账必填 | 用于验证 Waffo webhook 请求签名的 Base64 X.509 DER RSA 公钥。 |
| `WAFFO_TEST_MODE` | 否 | 标记 Waffo 配置为测试模式，默认 `false`。 |
| `WAFFO_PRODUCTS` | 开启时必填 | 固定产品 JSON 数组，例如 `[{"productId":"prod_xxx","name":"100k quota","quotaAmount":100000,"amountCents":1000,"currency":"usd"}]`。 |

行为说明：

- `GET /api/user/topup/waffo/config` 只返回非敏感配置状态和规范化后的产品列表。
- `POST /api/user/topup/waffo/checkout` 会为已配置产品创建待支付订单，并返回托管 checkout 地址。
- `POST /api/user/topup/waffo/webhook` 会验证 `X-SIGNATURE`、对已支付订单幂等入账，并把未支付的 pending 订单标记为失败。
- `WAFFO_PRODUCTS` 兼容 `productId` 或 `product_id`、`quotaAmount` 或 `quota`，金额字段兼容 `amountCents`、`priceCents` 或小数 `price`。
- 产品 JSON 无效时会被当作空目录处理，因此 Waffo 充值不会显示为已配置。
