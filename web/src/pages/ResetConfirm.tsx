  ...
  return (
    <main className="auth-page">
      <Card className="auth-card" bordered={false}>
        <Typography.Title heading={3}>{t('设置新密码')}</Typography.Title>
        <Typography.Paragraph type="tertiary">{t('使用邮件中的 token 完成密码更新。')}</Typography.Paragraph>
        ...
        >
          ...
          <Form.Input field="password" label={t('新密码')} mode="password" rules={[{ required: true }]} />
          <Button htmlType="submit" theme="solid" loading={loading} block>{t('更新密码')}</Button>
        </Form>
      </Card>
    </main>
  );
